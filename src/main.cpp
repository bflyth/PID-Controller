...
int main()
{
  ...
  PID steer_pid;
  PID throttle_pid;
  std::cout << "PID declared" << std::endl;
  
  double steer_Kp = -.125, steer_Ki = -.001, steer_Kd = -2.0;
  double throttle_Kp = -.2, throttle_Ki = 0.1, throttle_Kd = -2.0;

  steer_pid.Init(steer_Kp, steer_Ki, steer_Kd);
  throttle_pid.Init(throttle_Kp, throttle_Ki, throttle_Kd);
  std::cout << "PID values initialized" << std::endl;
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
		  ...
		  if (speed<10)
			  throttle_value = fmax(throttle_value, 0.7);          
          // DEBUG
